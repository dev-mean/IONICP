...
angular.module('openfb', [])
    ...
    .factory('OpenFB', function ($rootScope, $q, $window, $http) {
        ...
        function login(fbScope) {
            ...
            fbScope = fbScope || 'email, user_about_me, user_birthday, user_hometown, user_website,public_profile';

            deferredLogin = $q.defer();
            if (!fbAppId) {
                deferredLogin.reject('Facebook App Id not set.');
            }
            ...
            if (!oauthRedirectURL) {
                //alert('oauthCallback');
                if (runningInCordova) {//! for web view
                    ...
                    if (index > 0) {
                        ...
                        deferredLogin.reject('Can\'t reliably infer the OAuth redirect URI. Please specify it explicitly in openFB.init()');
